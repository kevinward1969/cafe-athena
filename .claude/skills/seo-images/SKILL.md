...
## Cafe Athena Project Notes

**Image location:** All hero and reference images live in `site/public/images/` as WebP files.

**Format:** Already WebP — do not recommend format conversion. Flag any non-WebP files as an anomaly.

**File size thresholds:** Food photography at 1920×1080 WebP 80% quality will commonly run 300–600KB. Apply these adjusted thresholds instead of the generic ones:
- Target: < 400KB
- Warning: 400–700KB
- Critical: > 700KB (flag for recompression)

**Alt text:** This is the primary gap to audit. Hero images in `[...slug].astro` likely have missing or generic alt text. Alt text for a recipe image should describe the dish — e.g. "Fool's Gold Tart with mirror-glaze surface and edible gold leaf on a marble surface."

**`fetchpriority="high"`:** The hero image on each recipe page is the LCP element — add this attribute if missing.

**Do not lazy-load** the hero image. It is always above the fold.

**CDN section:** Not applicable — site is served via FastComet with rsync, no CDN. Skip CDN recommendations.

**Responsive images / srcset:** Not currently implemented. Flag as a future improvement, not a blocker.
