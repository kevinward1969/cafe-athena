...
**Scope:** Apply only to free prose — headnotes, technique descriptions, glossary definitions, and web copy. Do NOT run on structured recipe sections (ingredient lists, Mise en Place, Method steps, Yield lines, Storage notes).

**Do not flag as AI-isms in this project:**
- **Em dashes** — used intentionally in culinary prose for rhythm and parenthetical asides. Retain them.
- **Title-case section headers** (`## Mise en Place`, `## Method`, `## The Sauce`) — these are intentional recipe conventions, not AI formatting.
- **Numbered/bulleted steps** in Method sections — structured steps are the required format for recipe methods, not a bullet-bloat problem.
- **Culinary terminology** that may sound "elevated" (e.g., "fold", "deglaze", "render", "emulsify") — domain vocabulary, not AI inflation.

**Voice target:** The writing should sound like a working chef — direct, confident, technically precise, no filler. Flag anything that sounds like marketing copy or hedged corporate writing.
