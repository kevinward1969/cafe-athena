...
**Primary schema type for this project is `Recipe` — it is not in the default list above but must be added to every recipe page.**
...
<script type="application/ld+json">
{
  ...
  "recipeCategory": "{{course — e.g. Dessert, Main Course}}",
  "recipeCuisine": "{{cuisine — e.g. French, Italian}}",
  "recipeYield": "{{yield from recipe}}",
  "recipeIngredient": [
    "{{ingredient 1}}",
    "{{ingredient 2}}"
  ],
  ...
  "keywords": "{{comma-separated keywords from recipe frontmatter}}"
}
</script>
```

**Required fields for Google rich results:** `name`, `image`, `recipeIngredient`, `recipeInstructions`.

**Recommended additional schemas per page:**
- `BreadcrumbList` — shows chapter path in SERP (e.g. Larder > Les Fonds > Foundation Tart Dough)
- `WebPage` — base page signals

**Implementation location:** Schema goes in the `<head>` of `site/src/pages/[...slug].astro`, injected dynamically from recipe frontmatter data (`Astro.props`).
