...
- **Content pipeline is custom.** Recipe Markdown source files live in `The Manual/` and are processed by `site/scripts/prepare-content.py` into `site/src/content/recipes/`. That directory is **ephemeral** — never suggest editing files there directly. All edits go in `The Manual/`.
- **Content collection config** is at `site/src/content.config.ts` (not `src/content/config.ts`).
- **Dynamic route** is `site/src/pages/[...slug].astro` (catch-all), not `[slug].astro`.
- **All output is static (SSG).** No SSR adapter, no `client:` directives, no JavaScript islands. Do not suggest SSR patterns or `client:load` unless explicitly requested.
- **Build command** is `cd site && npm run build` (runs `astro build` + `npx pagefind`). Never run `astro build` alone — Pagefind search index won't generate.
- **Hero images** live in `site/public/images/` as `XX-YY.webp`. The `heroImage` frontmatter field must be the filename only (e.g. `07-10.webp`) — never a path. The layout prepends `/images/` automatically.
