...
| **HF Tools (FLUX.1, Ideogram 4, Wan2.2)** | Lane 2 only — promotional stills, text-over-image assets, atmospheric video clips for social. Integration entry point: `~/Projects/Hugging Face/hugging_face/Projects/cafe-athena/hugging-face-agent.md`. All three tools are `skilled` — audit pending before production use. |
...
- **Image tool routing — two lanes, never mixed**: Lane 1 (cookbook photography) = Gemini Gem 2 only — hero images and reference images for `site/public/images/`. Lane 2 (promotional/social) = HF tools (FLUX.1 for atmospheric stills, Ideogram 4 for text-containing assets, Wan2.2 for video). Never route a cookbook hero image to HF tools; never route a promotional graphic to Gemini Gem 2. All three HF tools are audit-pending — do not use in production until AUDIT-004/AUDIT-005 are marked complete in `hugging_face/audit/`.
