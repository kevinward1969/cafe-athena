...
| **HF Tools** | Extended toolkit — supplements Adobe Firefly/Express and Gemini image generation for promotional and social content. Covers capabilities those tools don't own: atmospheric food stills (FLUX.1), text-in-image graphics (Ideogram 4), video clip generation and animated stills (Wan2.2), voiceovers (TTS tools), and music beds. Use HF when it is the best fit, not as the default for all promotional work. Integration entry point: `~/Projects/Hugging Face/hugging_face/Projects/cafe-athena/hugging-face-agent.md`. Audits pending before production use. |
...
| `~/Projects/Hugging Face/hugging_face/Projects/cafe-athena/hugging-face-agent.md` | HF tool integration entry point — tool registry, brand parameters, workflows for FLUX.1, Ideogram 4, and Wan2.2. Read-only from here; canonical skills live in the HF workspace. |
...
- **Generative tool routing**: Lane 1 (cookbook photography) = Gemini Gem 2 only — hero images and reference images for `site/public/images/`. Never route cookbook photography to any other tool. For promotional and social content, use the best tool for the task: Adobe Firefly and Adobe Express for design-centric graphics and brand compositions; Gemini image generation (Nanobanana) for photorealistic stills; HF tools when they are the strongest fit — FLUX.1 for atmospheric/editorial food stills, Ideogram 4 for text-in-image assets, Wan2.2 for video clips and panel-to-panel consistency, TTS tools for voiceovers, MusicGen/Stable Audio for music beds. HF tools extend the toolkit — they do not replace a better tool when one exists. All HF tools are audit-pending (AUDIT-004/AUDIT-005) — do not use in production until complete.
