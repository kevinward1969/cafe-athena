...
| Build `/pipeline [id]` slash command | Orchestrates full recipe pipeline from recipes.json state — registry-driven cascade, prompts before deploy. Image stage: check `site/public/images/XX-YY.webp` first; if present run size/spec check and continue; if absent stop and prompt. Single image location only (`site/public/images/`). CLAUDE.md "Two Image Workflows" section needs simplifying to single-location. Need Kevin's Photoshop output spec (dimensions + target file size) for size check thresholds. Validate on 09-06 then on first new recipe. | In design — build next session | 2026-05-18 |
